...
const MenuSection = ({ fetchData }) => {
  const uniqueThalis = [...new Set(fetchData.map((item) => item.thaliname))];
  
  // Initialize activeTab with the first uniqueThali item
  const [activeTab, setActiveTab] = useState('veg-thali');
  ...
  const activeData = fetchData.filter((data) => data.thaliname === activeTab);
                          ...
                          {activeData.map((data) => (
                            <div key={data.id} className="flex-shrink-0 border-l-2 border-r-2  border-white">
                              ...
                              <div className="px-6 py-2">
                                <h5 className="text-md font-bold text-white mb-2">{data.Foodname}</h5>
                                <h5 className="text-xl font-semibold text-white mb-2">
                                  <span className="text-base font-medium text-white">₹</span>{' '}
                                  {data.Foodcharge[0]?.price}/{data.Foodcharge[0]?.tenure}
                                </h5>
                                <a
                               href={`/arenechefview?id=${data.id}`}
