...
const Index = () => {
  ...
  const handleCloseAllInputFormats = () => {
    ...
    setFormData({
      ...
      Verified: 'false',
      roomTypes: [{ type: '', price: '',  images: [], videoSrc: null }],
    });
  ...
  useEffect(() => {
    if (user) {
        const db = getFirestore();
        const BanqueethallCollection = collection(db, 'Banqueethalldetail');
        const BanqueethallQuery = query(BanqueethallCollection, where('AgentId', '==', user.uid));

        // Real-time listener using onSnapshot
        const unsubscribe = onSnapshot(BanqueethallQuery, (snapshot) => {
            const data = [];
  ...
  console.log("Banqueethalldata",Banqueethalldata)
...
const handleDeleteVideo = (roomIndex) => {
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          if (selectedId) {
              await db.collection('Banqueethalldetail').doc(selectedId).update(dataWithMedia);
              toast.success('Update successful!', { position: toast.POSITION.TOP_CENTER });
          } else {
              await db.collection('Banqueethalldetail').add(dataWithMedia);
  ...
  const handleEdit = (data) => {
    setFormData({
      ...
      BanqueethallName: data.BanqueethallName ||  '',
      SeatingCapacity: data.SeatingCapacity || '',
      WithoutFoodPrice: data.WithoutFoodPrice || '',
      nonvegperplate: data.nonvegperplate || '',
      vegperplate: data.vegperplate || '',
      ...
      })) || [{ type: '', price: '', images: [], videoSrc: null }],
    });
      ...
      <div className="w-1/2">
          ...
          >
            ...
            <option value="Both Vegeterian & Non-Vegeterian">Both Vegeterian & Non-Vegeterian</option>
                  ...
                  <div className="w-1/2">
                   <input
                     ...
                     onChange={handleChange}
                     placeholder="Banqueethall Owner Name"
                    className="w-full p-2 border border-gray-300 rounded-md"
                   />
           ...
           <div className="  grid grid-cols-1 lg:grid-cols-4 gap-4">
             {['Food', 'Firecracker', 'Parking', 'WiFi', '24/7 Security', 'Aquaguard','Alcohal','DjSound','Airconditioner','Decoration'].map((benefit) => (
...
<button
                ...
                // Display the add Banqueethall Detail button when showAllInputFormats is false
                <div className="flex justify-end">
                    <button onClick={handleShowAllInputFormats} className="w-36 font-bold p-2 bg-blue-500 text-white rounded-md">
                        <span className="text-2xl font-bold">+</span> Add Banqueethall Detail
                    </button>
                </div>
            )}
    ...
    {Banqueethalldata.length === 0 ? (
        ...
        Banqueethalldata.map((item) => (
            <div key={item.id} className="w-full bg-gray-100 dark:bg-gray-800 border-gray-800 shadow-md hover:shadow-lg rounded-md">
                ...
                <a href={`/Banqueethalldetail?id=${item.id}`}>
