...
const Index = () => {
  ...
  useEffect(() => {
    if (user) {
        const db = getFirestore();
        const pgCollection = collection(db, 'pgdetail');
        const pgQuery = query(pgCollection, where('AgentId', '==', user.uid));

        // Real-time listener using onSnapshot
        const unsubscribe = onSnapshot(pgQuery, (snapshot) => {
            const data = [];
...
const handleDeleteVideo = (roomIndex) => {
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          if (selectedId) {
              await db.collection('pgdetail').doc(selectedId).update(dataWithMedia);
              toast.success('Update successful!', { position: toast.POSITION.TOP_CENTER });
          } else {
              await db.collection('pgdetail').add(dataWithMedia);
...
<button
                ...
                // Display the add PG Detail button when showAllInputFormats is false
                <div className="flex justify-end">
