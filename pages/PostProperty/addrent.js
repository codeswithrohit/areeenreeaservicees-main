...
const Index = () => {
  ...
  const handleCloseAllInputFormats = () => {
    ...
    setFormData({
      ...
      Propertyname: '',
      Verified: 'false',
      propertytypes: [{ type: '', price: '', availability: '', images: [], videoSrc: null }],
    });
  ...
  useEffect(() => {
    if (user) {
        const db = getFirestore();
        const RentCollection = collection(db, 'rentdetail');
        const RentQuery = query(RentCollection, where('AgentId', '==', user.uid));

        // Real-time listener using onSnapshot
        const unsubscribe = onSnapshot(RentQuery, (snapshot) => {
            const data = [];
...
const handleDeleteVideo = (roomIndex) => {
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          if (selectedId) {
              await db.collection('rentdetail').doc(selectedId).update(dataWithMedia);
              toast.success('Update successful!', { position: toast.POSITION.TOP_CENTER });
          } else {
              await db.collection('rentdetail').add(dataWithMedia);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const updatedData = Rentdata.filter((item) => item.id !== id);
  ...
  <div className="bg-white w-full  p-8 rounded-lg shadow-lg overflow-auto max-h-full">
           ...
           <div className="w-1/2">
                   ...
                   <input
                     ...
                     onChange={handleChange}
                     placeholder="eg: 1(Out of 2 Floors)"
                    className="w-full p-2 border border-gray-300 rounded-md"
                   />
                   </div>
                  </div> 
           ...
           <div className="  grid grid-cols-1 lg:grid-cols-4 gap-4">
             {['Food', 'Laundry', 'Parking', 'WiFi', '24/7 Security', 'Aquaguard','Furnished'].map((benefit) => (
...
<button
                ...
                <div className="flex justify-end">
                    <button onClick={handleShowAllInputFormats} className="w-36 font-bold p-2 bg-blue-500 text-white rounded-md">
                        <span className="text-2xl font-bold">+</span> Add Rent Detail
                    </button>
                </div>
            )}
