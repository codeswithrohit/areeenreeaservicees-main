...
const Index = () => {
  ...
  useEffect(() => {
    if (user) {
        const db = getFirestore();
        const ResortCollection = collection(db, 'Resortdetail');
        const ResortQuery = query(ResortCollection, where('AgentId', '==', user.uid));

        // Real-time listener using onSnapshot
        const unsubscribe = onSnapshot(ResortQuery, (snapshot) => {
            const data = [];
...
const handleDeleteVideo = (roomIndex) => {
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          if (selectedId) {
              await db.collection('Resortdetail').doc(selectedId).update(dataWithMedia);
              toast.success('Update successful!', { position: toast.POSITION.TOP_CENTER });
          } else {
              await db.collection('Resortdetail').add(dataWithMedia);
...
<button
                ...
                // Display the add Resort Detail button when showAllInputFormats is false
                <div className="flex justify-end">
                    <button onClick={handleShowAllInputFormats} className="w-36 font-bold p-2 bg-blue-500 text-white rounded-md">
                        <span className="text-2xl font-bold">+</span> Add Resort Detail
                    </button>
                </div>
            )}
    ...
    {Resortdata.length === 0 ? (
        ...
        Resortdata.map((item) => (
            <div key={item.id} className="w-full bg-gray-100 dark:bg-gray-800 border-gray-800 shadow-md hover:shadow-lg rounded-md">
                ...
                <a href={`/Resortdetail?id=${item.id}`}>
