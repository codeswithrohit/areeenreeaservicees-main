...
const AreneChef = () => {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      const pgRef = firebase.firestore().collection('Laundry').doc(pgId);
      await pgRef.update({
  ...
  const renderStars = (count) => (
    ...
    useEffect(() => {
        ...
        const pgRef = db.collection('Laundry').doc(id);
    
        pgRef.get().then((doc) => {
            if (doc.exists) {
                const data = doc.data();
                setFetchedData(data);
                setQuantity(parseInt(data.GarmentTypes[0].noofgarments)); // Set initial quantity to the noofthalli of the first package
    ...
    const handleTenureClick = (index) => {
        ...
        setQuantity(parseInt(fetchedData.GarmentTypes[index].noofgarments)); // Set quantity to noofthalli of selected package
    };

    const handleBuyNow = () => {
        if (fetchedData && selectedTenure !== null) {
            const selectedTenureData = fetchedData.GarmentTypes[selectedTenure];
            const query = {
    ...
    const getCurrentPrice = () => {
        ...
        const baseQuantity = parseFloat(selectedTenureData.noofgarments);
                        ...
                        <div className="p-4 lg:max-w-7xl max-w-xl max-lg:mx-auto">
                            <div className="grid items-start grid-cols-1 lg:grid-cols-5 gap-12">
                                <div className="min-h-[200px] lg:col-span-3 bg-gradient-to-tr from-emerald-200 via-emerald-300 to-emerald-400 rounded-lg w-full lg:sticky top-0 text-center p-6">
                                    <img src="https://img.freepik.com/free-photo/close-up-arranging-clothes_23-2148857479.jpg?t=st=1735460871~exp=1735464471~hmac=1fd244b6bb6e8cc77cd4a8767efac0a84c18b5e46674d44ea9ae4686faebddd4&w=2000" alt={fetchedData.service} className="w-60 h-60 rounded object-cover mx-auto py-6" />
                                    <hr className="border-white border my-6" />
                                </div>

                                <div className="lg:col-span-2">
                                    <h2 className="text-2xl font-bold text-gray-800">{fetchedData.service}</h2>

                                    <div className="mt-4">
                                        <h3 className="text-xl font-bold text-gray-800">Select Service Delivery Time</h3>
                                        <div className="flex flex-wrap gap-4 mt-4">
                                            {fetchedData.GarmentTypes.map((charge, index) => (
                                                <button 
                                                    key={index}
                                                    type="button" 
