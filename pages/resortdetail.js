...
const ResortdetailsViewCard = () => {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      const pgRef = firebase.firestore().collection('Resortdetail').doc(pgId);
      await pgRef.update({
  ...
  const roomOptions = data.roomTypes ? data.roomTypes.map((room) => ({
    value: room.type,
...
BOOK NOW
  ...
  <div className="relative h-full flex flex-col">
    ...
    <div className="w-full sm:w-80">
      <DatePicker
        ...
        style={{ width: '100%' }}
        className="w-full sm:w-64 h-12 border border-gray-300 rounded-lg px-4 py-2 text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
      />
    </div>
    ...
    <div className="w-full sm:w-auto">
      <button
        ...
        className="w-full sm:w-64 h-12 text-lg font-medium text-white bg-gradient-to-r from-[#f27121] via-[#e94057] to-[#8a2387] rounded-lg shadow-md transition-transform transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-pink-300"
      >
        Book Now
      </button>
    </div>
  ...
  {[
    ...
    >
      ...
      {activeTab === tab.name && (
        ...
        <FaCheckCircle className="text-green-500 text-xl md:text-2xl" title="Verified" />
              ...
              <p className="text-gray-900 font-bold md:text-lg text-xs flex ml-4  ">  <FaMapMarkerAlt className="text-red-600 mr-1 mt-1" /> {data.nearby},{data.district}</p>
    ...
    <p className="mt-4 text-md text-gray-900">{data.description}</p>
