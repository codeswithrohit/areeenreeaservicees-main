...
const HotelDetailsViewCard = () => {
  ...
  useEffect(() => {
    ...
    pgRef.get().then((doc) => {
      if (doc.exists) {
        ...
        if (pgData.propertytypes?.length > 0) {
          setSelectedRoomType(pgData.propertytypes[0]);
        }
      } else {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      const pgRef = firebase.firestore().collection('rentdetail').doc(pgId);
      await pgRef.update({
...
console.log("selected room",selectedRoom)
  const roomOptions = data.propertytypes ? data.propertytypes.map((room) => ({
    value: room.type,
...
BOOK NOW
    ...
    <div className="flex flex-col mt-16 gap-2 w-full sm:w-96">
                <DatePicker
                  onChange={handleCheckInChange}
                  format="YYYY-MM-DD"
                  placeholder="Check-in Date"
                  style={{ width: '100%' }}
                  disabledDate={disabledDate}
                          className="w-full h-12 mb-4 border border-gray-300 rounded-lg px-4 py-2 text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                />
              </div>

    {/* Room Selection */}
    <div className="flex flex-col gap-2 mb-4 w-full sm:w-96">
    <select
  ...
  <div className="mt-4 overflow-x-auto">
    <table className="min-w-full border-collapse border border-gray-300">
      ...
      <tbody>
        {data.propertytypes?.map((room, index) => (
          <tr
            key={index}
